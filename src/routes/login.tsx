...
function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn("Backend authentication failed, simulating credentials matching logic", err);
      
      // Fallback matching logic for quick demo sandbox testing
      const matched = defaultProfiles.find(
        (p) => p.email.toLowerCase() === email.toLowerCase() && p.pass === password
      );

      // Check dynamically created organization admins
      const savedOrgAdmins = localStorage.getItem("mock_org_admins");
      const dynamicOrgAdmins = savedOrgAdmins ? JSON.parse(savedOrgAdmins) : [];
      const matchedOrgAdmin = dynamicOrgAdmins.find(
        (p: any) => p.email.toLowerCase() === email.toLowerCase() && password === "defaultPassword123!"
      );

      if (matched || matchedOrgAdmin) {
        let dummyUser: any = null;

        if (matchedOrgAdmin) {
          dummyUser = {
            id: `usr_org_admin_${Date.now()}`,
            name: matchedOrgAdmin.name,
            email: matchedOrgAdmin.email,
        ...
        localStorage.setItem("token", `dummy_jwt_token_${dummyUser.id}`);
        localStorage.setItem("active_user", JSON.stringify(dummyUser));
        window.dispatchEvent(new Event("storage_user_change"));

        toast.success("Welcome Back (Mock Sandbox)!", {
        ...
        toast.error("Authentication Failed", {
          description: "Invalid email, password or tenant workspace combination."
        });
      }
    } finally {
      setLoading(false);
